...
- 用户要求新建隔离项目或禁止公司框架依赖时，启用便携生成模式：除测试类和数据文件外，一并生成请求RO、响应VO/DTO和API客户端；包名、基类、注解和工具类只使用中性命名、JDK或公开依赖，不得带入来源公司的私有包或框架。
...
隔离便携模式只从来源读取接口契约，不复制来源测试框架。提取HTTP方法、路径、字段名、Java类型、泛型、必填约束和序列化名称后，在新命名空间重新生成RO、VO/DTO和API客户端。扫描生成目录，来源公司的包名前缀、私有基类、私有注解和私有工具类必须为0处。
...
当前JAR只渲染测试类和数据资源。便携模式还必须由Skill在同一目标项目补齐：

- 请求RO，保留已证实的字段名、类型和序列化映射；
- 响应VO/DTO，包括响应包装及其泛型数据类型；
- API客户端或接口定义，保留HTTP方法与路径，基础地址和鉴权通过通用配置注入；
- 仅包含JDK和公开依赖的构建文件与最小数据加载/断言支持。

这些契约文件必须进入最终清单或单独的便携产物清单，并接受编译与私有依赖扫描。
...
- 便携模式包含RO、VO/DTO、API客户端、测试代码和用例资源，且生成目录不存在来源公司的私有包名或框架引用。
